...
class PowerBall : public GameObject
{
	...
	public:
			...
			PowerBall(const std::string meshFilePath, Vector3 position);
			PowerBall(const std::string meshFilePath, Vector3 position, int gameMode);
			virtual ~PowerBall();
			...
			iMesh* GetMesh() { return this->mMesh; }
			...
			void SetForwardVector(Vector3 forward) { this->mForward = Vector3(forward); }
