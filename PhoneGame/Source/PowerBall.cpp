...
*/
PowerBall::PowerBall(const std::string meshFilePath, Vector3 position)
{
	this->mHealth		 = 100;
	...
	this->mDamping		 = 0.99995f;//0.9995f; //0.995
	this->mMass			 = 999;
	this->mSumAddedForce = Vector3(0,0,0);
	this->mRestitution   = 0.0f; //0.95f
	this->mForcePress	 = 180.0f; // 180.0f
	this->mInTheAir		 = true;	// we are dropped from air
	this->mFriction		 = 1.0f;	// this is in the opposite direction to velocity, if this is 0, then no friction (only damping will decrese the speed)
...
PowerBall::PowerBall(const std::string meshFilePath, Vector3 position, int gameMode)
{
	this->mHealth		 = 100;
	this->mMesh			 = GetGraphics()->CreateStaticMesh(meshFilePath.c_str(), position); 
	this->mRadius		 = 1.0f;
...
void PowerBall::Update(const float dt, bool clientBall)
{
	...
	*/
	...
	Vector3 temp = this->GetMesh()->GetPosition();
	...
	*/
	...
	direction.y = 0;
	temp = Vector3(newPosition.x, newPosition.y, newPosition.z);
	this->mTempPosition = newPosition;
	//this->GetMesh()->SetPosition(temp);
	temp = Vector3(direction.x, direction.y, direction.z);
...
void PowerBall::UpdateBallParentMode(Map* map)
{
		...
		newPosBall += pos;
		this->SetPosition(newPosBall);
		this->SetTempPosition(newPosBall);
...
void PowerBall::UpdatePost()
{
	...
	Vector3 direction = newPosition - oldPosition;
	direction.y = 0;
	Vector3 temp = Vector3(newPosition.x, newPosition.y, newPosition.z);
	this->GetMesh()->SetPosition(temp);
	temp = Vector3(direction.x, direction.y, direction.z);
	this->Rotate(direction);	
...
bool PowerBall::collisionWithPlatformSimple(Map* p, Vector3 &normalPlane)
{
	...
	MaloW::Array<MeshStrip*>* temp = NULL;//p->GetMesh()->GetStrips();
...
void PowerBall::Rotate(Vector3 direction)
{
	...
	around.Normalize();
	Vector3 aroundD3D = Vector3(around.x, around.y, around.z);
